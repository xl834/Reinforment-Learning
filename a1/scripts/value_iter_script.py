...
def valueIterationStep(Q):
    ...
    A = (R).T + np.array(tuple([Q0, Q1, Q2, Q3])).T
    B = (R).T + discount * (P @ Qmax).T
    print(A-B)

    return (R).T + np.array(tuple([Q0, Q1, Q2, Q3])).T

def valueIteration(initialQ, tolerance=0.01):
    pi = np.zeros(17)  
    V = np.zeros(17)
    iterId = 0
    epsilon = np.inf
    ...
    while(epsilon > tolerance):
        Q_t1 = valueIterationStep(Q)
        epsilon = np.abs(np.max(Q_t1 - Q))
        Q = Q_t1
        iterId += 1
    
    pi = np.argmax(Q, axis=1)
    V = computeVfromQ(Q, pi)

    return pi, V, iterId, epsilon

def main():
    pi, V, iterId, epsilon = valueIteration(initial_Q_pi)
